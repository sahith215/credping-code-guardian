...
const Login = () => {
  ...
  const toggleSignUpMode = () => {
    // Set flip direction for proper animation
    const newDirection = isSignUp ? 'to-signin' : 'to-signup';
    setFlipDirection(newDirection);
    
    // Start flipping animation
    setIsFlipping(true);
    
    // Wait for half the animation duration before changing the form content
    setTimeout(() => {
      setIsSignUp(!isSignUp);
    }, 400);
    
    // End the animation after the full duration
    setTimeout(() => {
      setIsFlipping(false);
    }, 800);
  };
  ...
  // Calculate card transform style based on mouse position (only when not flipping)
  ...
  const flipStyle = isFlipping ? {
    transform: flipDirection === 'to-signup' 
  ...
  return (
    <div className="min-h-screen bg-credping-black flex flex-col justify-center items-center p-4 md:p-8 page-transition">
      ...
      <div 
        ...
        onMouseMove={!isFlipping ? handleMouseMove : undefined}
        onMouseLeave={!isFlipping ? handleMouseLeave : undefined}
      >
        <div 
          style={combinedStyle}
          className="card-raised p-8 transform-style-3d relative"
        >
          <div className={`backface-hidden w-full ${isSignUp ? 'rotate-y-180' : ''}`}>
            {/* Card Content - dynamically rendered based on isSignUp state */}
            {/* Card Header */}
